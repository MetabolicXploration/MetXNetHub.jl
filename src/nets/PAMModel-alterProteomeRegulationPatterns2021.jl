...
function __PAMModel_alterProteomeRegulationPatterns2021_builder(build_args...)
    ...
    # ISSUE/QUESTION: can this me extended to multiple nutrients at the same time?
    Sider = "EX_glc__D_e_b"                     # nutrinet ider
    Sidx = findfirst(rxns2 .== Sider)
    uS_max = 8.9                               # [mmol/ h gCDW] max nutrient intake
    @assert uS_max > 0
    w_UE_coes[Sidx] = f_UE0 / uS_max            # [h / g mmol] 
    ...
    # MARK: MetNet
    return MetNet(;S=S5, b=b5, lb=lb5, ub=ub5, c=c5, rxns=rxns5, mets=mets5)
end

function _PAMModel_alterProteomeRegulationPatterns2021_builder(build_args...)

    # load
    net = __PAMModel_alterProteomeRegulationPatterns2021_builder(build_args...)
    ...
    linear_weights!(net, "BIOMASS_Ecoli_core_w_GAM_f", 1.0)
